...
function setCurrentSong(o)
{
  ...
  if(o === null){
    ...
    $(".currentalbumart").html("<a href='/browse/" + o.artist + "/" + o.album + "'>" + "<img src='/cover/" + o.artist + "/" + o.album + "' onerror='imgError(this);'/>" + "</a>"); // Set album art for current song with link to said album, or runs imgError() when no image is found.
...
function windowSize(){
  ...
  if(windowwidth > '750'){
    ...
    $("#contentcontainercontainer").css("width", (windowwidth * .75) - 5 + "px");
    ...
    $(".sidebar").css("width", (windowwidth * .25) - 20 + "px");    
  }
...
function mobileQueue(){
  ...
  if(document.getElementById('mobileoverlay').style.display == 'none'){
    ...
    $("#mobilesidebarcontainer").css("width", windowwidth - 80 + "px");
    $("#mobilesidebarcontainer").css("height", windowheight - 50 + "px");
...
// Runs when an album cover isn't found for the currently playing song.
function imgError(source){
  source.src = "/nocover.png";
  source.onerror = "";
  return true;
}
