...
function updateQueue(){
  ...
  // Checks if there are any songs in the Queue, if there aren't then the div with the id "emptyqueue" is displayed.
  if(document.getElementsByClassName('queuelistitem') === null){
    $("#emptyqueue").css({'display': 'block'});
  }
  else{
    $("#emptyqueue").css({'display': 'none'});
  }
...
function setCurrentSong(o)
{
  ...
  if(o === null){
    ...
    setTimeout(update, 5000); // check again in 5 seconds
  }
  else{
    ...
    $(".artistalbumtitle").html("<a href='/browse/" + o.safe_artist + "'>" + o.artist + "</a>" + " &mdash; " + "<a href='/browse/" + o.safe_artist + "/" + o.safe_album + "'>" + o.album + "</a>"); 
    ...
    if(o.timeleft > 0){
      setTimeout(update, o.timeleft * 1000); // When it's time to update, update.
    }
    else{
...
function windowSize(){
  ...
  if(windowwidth > '750'){
    ...
    $("#contentcontainercontainer").css("width", (windowwidth * 0.75) - 5 + "px");
