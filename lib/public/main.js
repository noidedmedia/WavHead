...
function windowSize(){
  ...
  if(windowwidth > '750'){
    ...
    $(".artlistcontainer").css("width", (windowwidth * 0.75) - 350 + "px");

    $("#contentcontainer").css("height", windowheight - 180 + "px");
  ...
  // Otherwise, do this:
    ...
    $("#somethingplaying").css("width", windowwidth - 100 + "px");
...
function mobileCurrentSong(o){
  if(o === null){
    ...
    var currentsongtitlewidth = currentsongtitle.offsetWidth + 1;
    var footerwidth = windowwidth - 100;
    
    if(windowwidth < '750' && footerwidth < currentsongtitlewidth){
      console.log("Swag.");
      console.log(footerwidth);
      console.log(currentsongtitlewidth);
      $("#currentsongtitle").html(o.title + "                " + o.title);
      $("#currentsongtitle").css("animation", "marquee 20s linear infinite");
      document.getElementById('currentsongtitlestyles').innerHTML = "@keyframes marquee{ 0% { text-indent: 0px; } 100% { text-indent: -" + (currentsongtitlewidth + 8) + "px; } }";
    }
    else{
      console.log("No Swag.");
    }
